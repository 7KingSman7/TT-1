...
import {
  setCurrentPageElectronicAction,
  setTotalPageElectronicAction,
...
export const ElPage = () => {
  ...
  useEffect(() => {
    if (product) {
      ...
      dispatch(
        setTotalPageElectronicAction({ totalPageElectronic: totalPages })
      )
    }
  }, [product, dispatch])
  ...
  const handlePagePrev = () => {
    if (currentPage > 1) {
      ...
      dispatch(
        setCurrentPageElectronicAction({ currentPageElectronic: newValue })
      )
    }
  }
  ...
  const filteredArrProd = filteredProducts(product, 'Электроника')
